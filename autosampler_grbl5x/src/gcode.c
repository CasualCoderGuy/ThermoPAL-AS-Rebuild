...
void gc_init()
{
  ...
  gc_state.modal.motion = 1;
  gc_state.modal.units = 2;
...
uint8_t gc_execute_line(char *line)
{
      ...
      case 'G':
        ...
        switch(int_value) {
          ...
          case 20:
            ...
            gc_block.modal.units = 1;
            break;
          case 21: 
            dword_bit = MODAL_GROUP_G6;
            gc_block.modal.units = 0;
            break;
          case 22:
            dword_bit = MODAL_GROUP_G6;
            gc_block.modal.units = 2;
            break;
          case 40:
  ...
  */
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    ...
    else if (gc_block.modal.units == UNITS_MODE_STEP) { recalc_feedrate = 1; }
  } else {
            ...
            */
            ...
            if (gc_block.modal.units == UNITS_MODE_INCHES) {
              ...
              # else
                for (idx=0; idx<N_AXIS; idx++) { // Axes indices are consistent, so loop may be used to save flash space.
              #endif
                if (ijk_words & bit(idx)) { gc_block.values.ijk[idx] /= settings.steps_per_mm[idx]; }
              }
            }
  ...
  */
  if (recalc_feedrate) {
    float dxs,dys,dzs,das,divd;
    dxs = pow(gc_block.values.xyz[AXIS_1]-gc_state.position[AXIS_1],2); // Delta x between current position and target
    dxs = pow(gc_block.values.xyz[AXIS_2]-gc_state.position[AXIS_2],2); // Delta y between current position and target
    dzs = pow(gc_block.values.xyz[AXIS_3]-gc_state.position[AXIS_3],2); // Delta y between current position and target
    das = pow(gc_block.values.xyz[AXIS_4]-gc_state.position[AXIS_4],2); // Delta y between current position and target
    divd = pow(settings.steps_per_mm[AXIS_1],2)*dxs+pow(settings.steps_per_mm[AXIS_2],2)*dys+pow(settings.steps_per_mm[AXIS_3],2)*dzs+pow(settings.steps_per_mm[AXIS_4],2)*das;
    if (divd != 0)
      gc_block.values.f *= (dxs+dys+dzs+das)/divd;
  }
  recalc_feedrate = 0;
  // if (gc_block.values.f > MAX_FEED_RATE)
  //   FAIL(STATUS_MAX_STEP_RATE_EXCEEDED)
