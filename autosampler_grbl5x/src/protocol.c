    ...
    while((c = serial_read()) != SERIAL_NO_DATA) {
      if ((c == '\n') || (c == '\r')) { // End of line reached
        ...
        if (line_flags & LINE_FLAG_OVERFLOW) {
          ...
          report_status_message(STATUS_OK); //printString("*\n")
        } else if (line[0] == '$') {
          // Grbl '$' system command
...
void protocol_exec_rt_system()
{
  ...
  rt_exec = sys_rt_exec_report;
  if (rt_exec) {
    if ((rt_exec & EXEC_REPORT_POS) && !(sys.state & (STATE_CYCLE | STATE_JOG)))
    {
      send_pos();
      system_clear_exec_report();
    }
  }
    ...
    if (rt_exec & (EXEC_MOTION_CANCEL | EXEC_FEED_HOLD | EXEC_SAFETY_DOOR | EXEC_SLEEP)) {
      ...
      if (!(sys.state & (STATE_ALARM | STATE_CHECK_MODE))) {
        ...
        if (rt_exec & EXEC_FEED_HOLD) {
          ...
          if (!(sys.state & (STATE_SAFETY_DOOR | STATE_JOG | STATE_SLEEP))) { sys.state = STATE_HOLD; send_state();}
        }
        ...
        if (rt_exec & EXEC_SAFETY_DOOR) {
          ...
          if (!(sys.suspend & SUSPEND_JOG_CANCEL)) {
            ...
            if (sys.state != STATE_SLEEP) { sys.state = STATE_SAFETY_DOOR; send_state();}
          }
