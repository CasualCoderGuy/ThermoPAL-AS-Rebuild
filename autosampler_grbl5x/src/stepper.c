...
#define JOG_REPORT_INCREMENT 20
#define POS_REPORT_INCREMENT_MS 500
uint8_t jog_pos_report_delay = 0;
    ...
    */
    ...
    if (mm_remaining == prep.mm_complete) {
      ...
      if (mm_remaining > 0.0) { // At end of forced-termination.
        ...
        if (sys.step_control & STEP_CONTROL_EXECUTE_SYS_MOTION) {
          bit_true(sys.step_control,STEP_CONTROL_END_MOTION);
          system_set_exec_report(EXEC_REPORT_POS);
          return;
        }
        pl_block = NULL; // Set pointer to indicate check and load next planner block.
        plan_discard_current_block();
        //if (settings.send_data & SEND_POS){}
        // if (sys.state & STATE_JOG) {
        //   if (jog_pos_report_delay >= JOG_REPORT_INCREMENT) { jog_pos_report_delay = 0; send_pos(); }
        //   else if (plan_get_block_buffer_available()) { jog_pos_report_delay++; }
        //   else { jog_pos_report_delay = 0; send_pos(); system_set_exec_report(EXEC_REPORT_POS); } //set flag here
        //   //printString("&\n");
        // }
        // else { send_pos(); system_set_exec_report(EXEC_REPORT_POS);}
        if (sys.state & STATE_JOG) {
          if (!plan_get_block_buffer_available()) { send_pos(); system_set_exec_report(EXEC_REPORT_POS); }
          printString("&\n");
        }
        else { send_pos(); system_set_exec_report(EXEC_REPORT_POS);}
      }
    }
    //report increment here
    if ((sys.state & (STATE_CYCLE | STATE_JOG)) && pl_block) {
      for (uint8_t idx = 0; idx < N_AXIS; idx++)
      {
        if (abs(sys_position[idx] - prev_pos[idx]) > (pl_block->programmed_rate*POS_REPORT_INCREMENT_MS/60000*settings.steps_per_mm[idx]))
        {
            for (uint8_t i=0; i < N_AXIS; i++) {
              prev_pos[i] = sys_position[i];
            }
            send_pos();
        }
      }
