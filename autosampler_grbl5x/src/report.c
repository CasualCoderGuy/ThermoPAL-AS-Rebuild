...
static void report_util_cr() { printString("\r"); }
static void report_util_nl() { printString("\n"); }
static void report_util_feedback_nl() { serial_write(']'); report_util_nl(); }
static void report_util_feedback_cr() { serial_write(']'); report_util_cr(); }
...
static void report_util_uint8_setting(uint8_t n, int val) {
  ...
  report_util_cr(); // report_util_setting_string(n);
}
...
void report_status_message(uint8_t status_code)
{
  switch(status_code) {
    case STATUS_OK: // STATUS_OK
      //printString("ok\n"); break;
      if (sys.state != STATE_JOG) {printString("ok\n");}
      break;
    default:
      printString("!");
...
{
  printPgmString(PSTR("\rGrbl Autosampler " GRBL_VERSION "." AS_VERSION " ['$' for help]"));
  report_util_nl();
}
...
void report_grbl_settings() {
  ...
  report_util_uint8_setting(35,settings.disable_reset);
  report_util_uint8_setting(40,settings.sig_out_invert_mask);
  report_util_uint8_setting(41,settings.sig_in_invert_mask);
  report_util_uint8_setting(42,settings.limit_invert_mask);
  // Print axis settings
  uint8_t idx, set_idx;
  ...
  for (set_idx=0; set_idx<AXIS_N_SETTINGS; set_idx++) {
    for (idx=0; idx<N_AXIS; idx++) {
      switch (set_idx) {
        ...
        case 4: report_util_float_setting(val+idx,settings.ratio[idx],N_DECIMAL_SETTINGVALUE); break;
      }
    }
...
void report_gcode_modes()
{
  ...
  report_util_gcode_modes_G();
  print_uint8_base10(20+(gc_state.modal.units^((gc_state.modal.units >> 1) ^ 1)));
...
void report_build_info(char *line)
{
  ...
  printPgmString(PSTR("[VER=" GRBL_VERSION "." AS_VERSION "." AS_VERSION_BUILD ":"));
  printString(line);
  report_util_feedback_cr();
