...
uint8_t limits_get_state()// ^ settings.limit_invert_mask)
{
...
uint8_t homing_init()
{
  if (bit_isfalse(settings.flags,BITFLAG_HOMING_ENABLE)) {send_pos(); return(STATUS_SETTING_DISABLED); }
...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  #ifdef LIMIT_2_STATE
    uint8_t limit_state_start;
    limit_state_start = limits_get_state();
  #endif
  do {
      ...
      #endif
      {
        ...
        for (idx=0; idx<N_AXIS; idx++) {
          if (axislock[idx] & step_pin[idx]) {
            #ifdef LIMIT_2_STATE
            if ((limit_state_start & (1 << idx)) ^ (limit_state & (1 << idx))){
            #else
            if (limit_state & (1 << idx)) {
            #endif
              #ifdef COREXY
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    if (cycle_mask & bit(idx)) {
        ...
        if (idx==TOOL_LENGTH_OFFSET_AXIS)
          sys_position[idx] = set_axis_position+(int32_t)trunc((syringe.needle_offset)*settings.steps_per_mm[idx]);
        else
          sys_position[idx] = set_axis_position;
      #endif
