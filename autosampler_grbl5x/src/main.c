...
volatile uint8_t sys_rt_exec_report;
uint8_t homing_init_error = STATUS_IDLE_ERROR;
...
int main(void)
{
  ...
  // things uncontrollably. Very bad.
    ...
    if (bit_istrue(settings.flags,BITFLAG_HOMING_ENABLE)) { sys.state = STATE_ALARM; send_state();}
  #endif
  ...
  for(;;) {
    ...
    //uint8_t prior_as_state = sys.as_state;
    memset(&sys, 0, sizeof(system_t)); // Clear system struct variable.
    sys.state = prior_state;
    //if (prior_as_state & STATE_AS_PARK) {sys.as_state = prior_as_state;}
    send_state();
    ...
    if (homing_init_error)
      homing_init_error = homing_init();
    report_status_message(homing_init_error);
